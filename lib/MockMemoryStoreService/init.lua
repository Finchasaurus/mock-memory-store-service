...
function MockMemoryStoreService:GetQueue(name: string, timeout: number?)
	MockMemoryStoreUtils.WarnOnce(
		"MockMemoryStoreService queue is still in development, and may not work accurately just yet."
	)
...
function MockMemoryStoreService:GetHashMap(name: string)
	...
	if hashMap == nil then
		hashMap = MockMemoryStoreHashMap.new()
